...
int send_dhcp_discover(int sock)
{
    ...
    /**** WHAT THE HECK IS UP WITH THIS?!?  IF I DON'T MAKE THIS CALL, ONLY ONE SERVER RESPONSE IS
     * PROCESSED!!!! ****/
    /* downright bizzarre... */
...
int get_dhcp_offer(int sock)
{
    ...
    for (responses = 0, valid_responses = 0;;)
    {
        ...
        result = OK;
        result = receive_dhcp_packet(
            &offer_packet, sizeof(offer_packet), sock, dhcpoffer_timeout, &source);
...
int receive_dhcp_packet(
    void* buffer, int buffer_size, int sock, int timeout, struct sockaddr_in* address)
{
    struct timeval tv;
    fd_set readfds;
    fd_set oobfds;
    ...
    else
    {
        ...
        recv_result = recvfrom(
            sock, (char*)buffer, buffer_size, 0, (struct sockaddr*)&source_address, &address_size);
        if (verbose)
    ...
    for (x = 4; x < MAX_DHCP_OPTIONS_LENGTH - 1;)
    {
        ...
        switch (option_type)
        {
            ...
            case DHCP_OPTION_REBINDING_TIME:
                memcpy(
                    &dhcp_rebinding_time, &offer_packet->options[x], sizeof(dhcp_rebinding_time));
...
int get_results(void)
{
    ...
    else
    {
        ...
        for (temp_offer = dhcp_offer_list; temp_offer != NULL; temp_offer = temp_offer->next)
        {
            ...
            /* see if we got the address we requested */
            if (!memcmp(
                    &requested_address, &temp_offer->offered_address, sizeof(requested_address)))
                received_requested_address = TRUE;
        }
    }
    ...
    if (request_specific_address == TRUE)
        printf(_(", requested address (%s) was %soffered"), inet_ntoa(requested_address),
...
int call_getopt(int argc, char** argv)
{
    ...
    static struct option long_options[] = { { "serverip", required_argument, 0, 's' },
        { "requestedip", required_argument, 0, 'r' }, { "timeout", required_argument, 0, 't' },
        { "interface", required_argument, 0, 'i' }, { "mac", required_argument, 0, 'm' },
        { "unicast", no_argument, 0, 'u' }, { "verbose", no_argument, 0, 'v' },
